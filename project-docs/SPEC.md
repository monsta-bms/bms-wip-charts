...
解析済みファイル欄は、状態、外側ファイル名、補助行、block数・サイズ、変更・解除の共通骨格を使う。ZIPでは補助行へ内部BMS名を表示し、単体BMS/BME/BMLでは読み上げ対象外の空領域を保つことで、両形式のファイル名開始位置と縦位置を揃える。長い名前は省略し、titleで全文を確認できるようにする。
