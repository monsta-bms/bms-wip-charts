...
check("completed badge color is shared by top and list", () => assert.match(sources.sharedCss, /\.progress-pill\.is-completed\s*\{[\s\S]*var\(--primary\)/u));
...
check("compact comments receive the expanded flexible track", () => assert.match(sources.listCss, /minmax\(320px, 1\.75fr\) max-content/u));
check("compact actions shrink-wrap on desktop", () => assert.match(sources.listCss, /\.compact-actions-cell\s*\{[\s\S]*width:\s*max-content;/u));
...
assert.ok(passed >= 21, `expected at least 21 checks, got ${passed}`);
