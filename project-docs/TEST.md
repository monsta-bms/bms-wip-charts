...
- 単体BMS/BME/BMLとZIPの解析済み表示で、状態、外側ファイル名、block数・サイズ、変更・解除の位置が揃うこと。ZIPだけ内部BMS名が表示され、単体譜面の空補助行は読み上げ対象にならないこと。
...
- 完成版ボタンが0～79%でdisabled、80～99%で有効、100%・没譜面・未解析で非表示の既存条件を維持すること。
- 進捗操作帯の左右端と進捗レールが揃い、進捗度、数値、%、補足文、完成版ボタンが縦中央で読めること。
...
- 差分情報が「譜面情報」と「作者情報」に分かれ、譜面情報に難易度と仮差分名、作者情報に差分作者と補足文があること。
- 760px以下では譜面情報、想定難易度、仮差分名、作者情報、差分作者の順に1列表示されること。
- 難易度の未選択、★、★★、sl、st、手入力、縮小、変更、追記初期値、reset、validationが従来どおり動くこと。
- 進捗度と完成版ボタンが進捗グラフ・レール・小節ラベルの直下にあり、没譜面がその次の行にあること。
- 完成版ボタンが未解析/100%/没譜面で非表示、0～79%でdisabled、80～99%で有効のままであること。
...
- 1366pxで未選択時の差分情報が難易度左列、仮差分名・作者右列になり、選択後は3列へ圧縮されること。
- 1024pxでは2列、760px以下では1列になり、手入力と追記モードでも入力欄が重ならないこと。
- 進捗度inputが約88pxで進捗マップ上部にあり、`%`、完成版ボタン、没譜面の関係が読み取れること。
- 完成版ボタンが未解析/100%/没譜面で非表示、80%未満でdisabled、80～99%で有効の既存条件を維持すること。
...
- helper textが短文化されても、原曲URL、仮差分名、進捗、没譜面、管理パスワード、保存設定、コメントの意味が維持されること。
...
- 追記投稿では親の楽曲情報・差分名・progress layerを維持し、今回選択したファイルの格子一致後だけ進捗編集を表示すること。
- ファイル未選択時の進捗欄には案内文だけが表示され、解析中は解析状態、成功後だけグラフ・編集ブロック・進捗度・没譜面・完成操作が表示されること。
- 完成操作が80%未満でdisabled、80〜99%で有効、100%または没譜面で非表示になること。
