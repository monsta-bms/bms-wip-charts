...
## TOP-RECENT-SUCCESS-05 確認項目

- 通常トップが`charts.updated_at DESC`順の6chartだけを表示し、同じchartが重複しないこと。
- トップに検索、クリア、お気に入りのみ、全件件数、追加読み込みがなく、`すべての投稿を見る`が`list.html`を開くこと。
- chartカード内のツリー、折り畳み、進捗サムネイル、miniView、DL、追記、お気に入り、管理UIが従来どおり動作すること。
- 詳細URLでは「選択中の投稿」の後に最近6chartが表示され、選択chartは最近一覧から除外されること。
- 通常表示へ戻るリンクで`chartId`/`versionId`が外れ、通常トップへ戻ること。
- 初回投稿成功後、作成したBASEを含むchartが取得され、対象行のfocus・scroll・強調が行われること。
- 追記投稿成功後、深い分岐、同じ親からの分岐、完成版を含む作成versionが正確に選択されること。
...
- 320px、390px、760px、1366px、1920pxで横スクロールがなく、見出しと一覧リンク、選択中カードが読めること。
