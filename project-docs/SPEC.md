...
想定難易度欄は、テキスト入力ではなく「シンボルタブ + 数字チップ式UI」とする。フォーム全体の現行配置は維持し、想定難易度欄だけを選択UIへ置き換える。
...
ユーザー向けに上限説明文は表示しない。上限は選択できる数字チップを制限することで表現する。
...
手入力モードでは、数字チップではなく自由入力欄を表示する。シンボル込み入力を許可し、入力中の値をプレビューに反映する。数字部分が3桁以上になる入力は受け付けない。手入力時はシンボル別の上限チェックは行わない。
