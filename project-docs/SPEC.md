...
PROG-03Bでは、視認性と操作性を優先し、折れ線グラフと編集ブロックの重ね合わせを廃止する。進捗マップは以下の2層構造に分ける。

1. 上段: 1秒ごとのプレイノート密度を示す棒グラフ。読み取り専用。
2. 下段: 進捗を塗るための標準化ブロック。クリック/ドラッグで編集可能。

今回の進捗マップはフロント試作であり、API送信項目は増やさない。既存の `progress` 入力欄へ算出値を反映するだけとする。
...
- `#BPM`, `#BPMxx`, `#xxx03`, `#xxx08` を読み、1秒密度グラフの時間推定に使う。
...
上段のグラフは、BMS内のプレイノートを推定秒数へ変換し、1秒ごとのノート数を棒グラフで表示する。
...
- 緑色は「作成済みに塗ったブロック」のみに使う。
- 8ブロックごとの区切り線は濃いグレーで表示する。
- 通常のブロック境界線は薄いグレーで表示する。
- 1秒密度グラフと進捗ブロックは上下に分離し、意味が混ざらないようにする。
- 進捗マップ付近には `play notes: xxxx / blocks: xx / progress: xx%` を表示する。
- 必要に応じて `measures: first-last` も表示する。

### 塗り操作

- 未塗りブロックをクリックすると塗られる。
- 塗り済みブロックをクリックすると解除される。
- 未塗りブロックからドラッグを開始した場合、通過したブロックを連続して塗る。
- 塗り済みブロックからドラッグを開始した場合、通過したブロックを連続して解除する。
...
手入力欄はPROG-03Bでは残す。将来、進捗マップが安定したら手入力を廃止する可能性がある。
...
PROG-03Bでは、`completed_at` 保存、`progress_map_json` 保存、進捗画像保存は行わない。
...
PROG-03Bでは保存しないが、後続Phaseでは以下を使う。
