...
function projectMeasureNotes(value: string | null, versionId: string): {
  ...
  const ready = miniViewRecord.status === "ready"
    ...
    && [2, 3].includes(Number((miniViewRecord.payload as Record<string, unknown>).schemaVersion));
  return {
    measureNotes: {
      ...root,
  ...
  try {
    ...
    if (input.extension === ".zip") {
      ...
      try {
        ...
        console.error("[create-chart-zip-inspection] unexpected ZIP inspection failure", {
