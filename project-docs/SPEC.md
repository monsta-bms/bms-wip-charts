...
譜面ファイルセクションには、譜面ファイル入力と対応形式説明を表示する。

楽曲情報セクションには、以下を2カラムで表示する。
...
想定難易度UIは差分情報セクション内で広めに扱う。仮差分名と差分作者は想定難易度の下に並べる。仮差分名は、同じ曲の別差分を区別する短い識別名として扱い、長文textareaのような高さにはせず、曲名やアーティスト欄に近い1行テキスト入力の高さにする。

進捗・管理セクションには、以下をまとめる。
...
進捗・管理セクションは、将来の進捗グラフUI追加を見据えて過度に作り込まない。進捗度の現在の挙動、没譜面ON時の `progress=100` 固定、パスワード保存は維持する。

コメントセクションにはコメント欄をフォーム下部に広めに表示する。
...
想定難易度欄は、テキスト入力ではなく「シンボルタブ + 数字チップ式UI」とする。フォーム全体の配置は意味単位を優先し、サブアーティストの直後に仮差分名が来て、その右側に想定難易度があるような流れにはしない。
