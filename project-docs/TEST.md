...
- 通常のブロック境界線が薄いグレーで表示されること
- 未塗りブロックをクリックすると塗られること
- 塗り済みブロックをクリックすると解除されること
- 未塗りブロックからドラッグすると連続して塗れること
- 塗り済みブロックからドラッグすると連続して解除できること
...
#00301:01010101
#00303:120
"@ | Set-Content -Encoding UTF8 .\prog03b-progress-map-test.bms
```
...
- いずれのファイルでも、ドラッグ塗りとドラッグ解除ができること

## GitHub PagesでのPROG-03B確認
...
4. `prog03b-progress-map-test.bms` または添付BMSを選択する。
...
10. 未塗りブロックを1つクリックし、進捗度欄が更新されることを確認する。
11. 同じブロックをもう一度クリックし、塗りが解除されることを確認する。
12. 未塗りブロックからドラッグして、複数ブロックを連続して塗れることを確認する。
13. 塗り済みブロックからドラッグして、複数ブロックを連続して解除できることを確認する。
14. 8ブロックごとの濃いグレー線が表示されることを確認する。
15. progressが80%以上かつ100%未満になると `完成版にする` ボタンが有効になることを確認する。
16. `完成版にする` を押す。
17. 未塗りブロックがすべて塗られ、進捗度欄が `100` になることを確認する。

## 没譜面との連動確認

1. `prog03b-progress-map-test.bms` または添付BMSを選択する。
2. 没譜面チェックをONにする。
...
#00101:01010101
#00103:120
"@ | Set-Content -Encoding UTF8 .\prog03b-no-notes.bms
```

確認手順:

1. `prog03b-no-notes.bms` を選択する。
...
1. GitHub Pages画面を開く。
2. `prog03b-progress-map-test.bms` または添付BMSを選択する。
...
curl.exe -X POST "http://localhost:8787/api/charts" `
  -F "file=@.\prog03b-progress-map-test.bms;type=text/plain" `
  -F "title=Progress Map Test" `
  -F "subtitle=" `
  -F "artist=Test Artist" `
  -F "subartist=" `
  -F "chartName=PROG-03B Test" `
  -F "difficulty=★12" `
  -F "level=12" `
  -F "author=tester" `
  -F "progress=50" `
  -F "comment=PROG-03B test" `
  -F "isRejected=false" `
