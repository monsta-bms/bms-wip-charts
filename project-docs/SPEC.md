...
- 投稿一覧はchart単位でページングし、Pagesでは1回20件ずつ取得する。
...
- Pagesは「さらに読み込む」で次ページを取得し、chart IDで重複を除去してから累積データ全体を再描画する。
- 検索条件の変更時は1ページ目へ戻し、古い通信は中断または世代番号で無視する。
- 検索語はURLの`q`に保持する。追加取得ページ番号はURLへ保持せず、再読み込み時は1ページ目から開始する。
- お気に入り状態とお気に入りのみ表示は取得済みchartに対して従来どおり適用する。localStorage上にある未取得ページのお気に入りを直接取得する機能は別フェーズとする。
- OFFSET方式の途中で投稿順が変化した場合に備え、Pagesはchart IDによる重複除去を行う。将来、件数増加時はカーソル方式を検討する。
