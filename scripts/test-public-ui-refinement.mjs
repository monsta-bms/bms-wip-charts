...
check("home hero explains the service", () => {
  assert.match(sources.index, /BMS差分共有サイト/u);
  assert.match(sources.index, /未完成譜面・没譜面を共有する/u);
...
check("public version action is labeled 投稿操作", () => {
  assert.match(sources.actions, /button\.textContent = "投稿操作"/u);
  assert.match(sources.actions, /投稿後の操作を開く/u);
});
check("public dialog heading is 投稿後の操作", () => {
  assert.match(sources.index, /id="versionManagementTitle">投稿後の操作/u);
  assert.match(sources.management, /`投稿後の操作: \$\{state\.versionLabel\}`/u);
});
...
check("RC uses accepted intro and action labels", () => {
  ...
  assert.match(sources.rc, />コメント<\/summary>/u);
});
...
check("spec records the accepted public UI contract", () => assert.match(read("project-docs/SPEC.md"), /## PUBLIC-UI-REFINEMENT-01/u));
