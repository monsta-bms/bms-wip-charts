...
- 想定難易度欄がテキスト入力ではなく、シンボルタブ + 数字チップUIになっていること
- シンボルタブは `★` / `★★` / `sl` / `st` / `手入力` が表示されること
- `★` 選択時は 1〜25 が選べること
- `★★` 選択時は 1〜7 が選べること
- `sl` 選択時は 1〜12 が選べること
- `st` 選択時は 1〜15 が選べること
- 手入力は2桁までの数字を含む文字列を入力できること
- ユーザー向けに上限説明文が表示されていないこと
...
- 手入力で `★12` を入力すると `difficulty="★12"`, `level="12"` になること
- 手入力で `★★7` を入力すると `difficulty="★★7"`, `level="7"` になること
- 手入力で `sl10` を入力すると `difficulty="sl10"`, `level="10"` になること
- 手入力で `st15` を入力すると `difficulty="st15"`, `level="15"` になること
- 手入力で `12` を入力すると `difficulty="12"`, `level="12"` になること
- 手入力で数字を含まない文字列を入れた場合、`difficulty` は保存され、`level` は空または `NULL` になること
- 手入力で数字部分が3桁以上になる入力は拒否される、または3桁目が入力できないこと
- 手入力時はシンボル別の上限チェックを行わないこと
- 通常のシンボルタブ選択時は、シンボル別上限制御が働くこと
- 必須項目に赤い `*` が表示されること
- `*項目は入力必須。` の説明が表示されること
- 初期表示時に赤エラーが出すぎないこと
...
2. 投稿フォーム全体の配置が現行UIから大きく変わっていないことを確認する。
3. 想定難易度欄だけがシンボルタブ + 数字チップUIに置き換わっていることを確認する。
4. 初期表示時に上部エラー欄や各入力欄が赤エラーだらけになっていないことを確認する。
5. 譜面ファイル、曲名、アーティスト、仮差分名、想定難易度、差分作者、進捗度、管理パスワードに赤い `*` が表示されることを確認する。
...
2. `★` タブで `12` を押し、プレビューが `★12` になることを確認する。
3. `★` タブで 1〜25 だけが表示されることを確認する。
4. `★★` タブで 1〜7 だけが表示されることを確認する。
5. `sl` タブで 1〜12 だけが表示されることを確認する。
6. `st` タブで 1〜15 だけが表示されることを確認する。
...
10. タブと数字チップがbutton要素として操作でき、選択中状態が見た目と `aria-pressed` で分かることを確認する。
11. ユーザー向けに `★は25まで` のような上限説明文が表示されていないことを確認する。

## 手入力確認

1. `手入力` タブを選ぶ。
2. 数字チップが消え、自由入力欄が表示されることを確認する。
3. placeholderが `例: ★12 / sl10 / 自由入力` であることを確認する。
4. `★12` を入力し、プレビューが `★12` になることを確認する。
5. `★★7` を入力し、プレビューが `★★7` になることを確認する。
6. `sl10` を入力し、プレビューが `sl10` になることを確認する。
7. `st15` を入力し、プレビューが `st15` になることを確認する。
8. `12` を入力し、プレビューが `12` になることを確認する。
9. `overjoy` を入力し、プレビューが `overjoy` になることを確認する。
10. `100` や `st100` のように数字が3桁以上になる入力が拒否される、または3桁目が入力できないことを確認する。
11. 手入力時は `★★9` や `sl15` のようなシンボル別上限外の表記も、3桁数字でなければ入力自体は可能であることを確認する。
...
@"
#PLAYER 1
#TITLE Test Song Difficulty Picker
#ARTIST Test Artist
#PLAYLEVEL 3
#BPM 120
#00111:01
"@ | Set-Content -Encoding UTF8 .\difficulty-picker-test.bms
```

## BMSメタデータ自動読取確認

1. GitHub Pages画面を開く。
2. `difficulty-picker-test.bms` を選択する。
...
```bash
cd worker
npx wrangler d1 execute wip-bms-charts-db --command "SELECT difficulty, level FROM versions WHERE title LIKE '%Difficulty Picker%' ORDER BY created_at DESC LIMIT 5;"
```
...
DELETE FROM versions WHERE title LIKE '%Difficulty Picker%';
DELETE FROM charts WHERE chart_name IN ('[REJECTED]', '[NORMAL]', '[INVALID]', '[ANOTHER]', '[ALITHER]', '仮差分');
DELETE FROM songs WHERE title LIKE '%Difficulty Picker%';
```
