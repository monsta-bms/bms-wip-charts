...
- 上段に表示範囲内のプレイノート密度が棒グラフで表示されること
...
6. 上段に表示範囲内のノート密度が棒グラフで表示されることを確認する。
...
16. `prog03c-offset-test.bms` を選択し、表示左端が `004` 相当になることを確認する。
17. `prog03c-offset-test.bms` で、曲頭の進捗対象外空白が通常表示に混ざらないことを確認する。
18. `prog03c-offset-test.bms` で、上段の棒グラフと下段ブロックが同じ横位置を指していることを確認する。
19. 未塗りブロックを1つクリックし、進捗度欄が更新されることを確認する。
20. 同じブロックをもう一度クリックし、塗りが解除されることを確認する。
21. 未塗りブロックから3ブロック以上ドラッグして、範囲塗りできることを確認する。
22. そのまま左クリックを押した状態で戻し、余分な塗りが解除されることを確認する。
23. 塗り済みブロックから3ブロック以上ドラッグして、範囲解除できることを確認する。
24. そのまま左クリックを押した状態で戻し、解除しすぎた部分が元に戻ることを確認する。
25. ブロックにマウスを乗せ、小節範囲、秒数範囲、notesのtooltipが出ることを確認する。
26. ブロックを右クリックし、簡易情報ポップアップが出ることを確認する。
27. ブロック右クリック時に通常のブラウザメニューが出ないことを確認する。
28. 8ブロックごとの濃いグレー線と通常境界線が見分けられることを確認する。
29. progressが80%以上かつ100%未満になると `完成版にする` ボタンが有効になることを確認する。
30. `完成版にする` を押す。
31. 未塗りブロックがすべて塗られ、進捗度欄が `100` になることを確認する。
