...
FORM-02以降、投稿フォーム下段は左右カラムを独立した縦積みにし、右カラムの想定難易度UIの高さで左カラムの入力欄が引き伸ばされないようにする。

左カラムは「差分識別・投稿者補助情報」のまとまりとして以下の順に表示する。
...
仮差分名は、同じ曲の別差分を区別する短い識別名として扱う。長文textareaのような高さにはせず、曲名やアーティスト欄に近い1行テキスト入力の高さにする。補足文は残すが、仮差分名、差分作者、没譜面、パスワード保存が左カラム内で自然に詰まって見える余白にする。
