...
import { CHANGELOG_FILENAME, GUIDE_FILENAME, readUtf8, SiteCopyError, UI_FILENAME, validateEditedCopies } from "./site-copy-core.mjs";

export function validationOptions(argv) {
  const rootDir = path.resolve(import.meta.dirname, "../..");
  const outputDir = "C:\\Users\\longa\\Documents\\Tools\\bms-wip-charts-copy";
  const options = { rootDir, uiPath: path.join(outputDir, UI_FILENAME), guidePath: path.join(outputDir, GUIDE_FILENAME), changelogPath: path.join(outputDir, CHANGELOG_FILENAME), snapshotPath: null };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (argv[index] === "--changelog") options.changelogPath = path.resolve(argv[++index]);
...
export function runValidation(options) {
  ...
  return validateEditedCopies(readUtf8(options.uiPath), readUtf8(options.guidePath), snapshot, { rootDir: options.rootDir, changelogText: readUtf8(options.changelogPath) });
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(import.meta.filename)) {
  try {
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      changelogEntryCount: result.changelogEntryCount,
      ...
      changelogEntries: result.changelogChanges.map((change) => change.entry.id)
    }, null, 2)}\n`);
  } catch (error) {
